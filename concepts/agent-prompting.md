...
Beyond the static template, agent prompts are assembled dynamically at runtime. Data the agent needs is injected into the prompt just before the call: "the meeting is going to be passed into the agent and then added onto the prompt like this right here" ([0:06:24]), and "you can even give roles and constraints there's so many different levels of customization" ([0:06:37]). A critical piece of that runtime context is grounding the model in the present moment, "we want to tell our assana assistant what day it is so it can accurately determine due dates when we give relative dates like this Saturday or next Friday" ([0:05:47]).
...
- [You're WASTING TIME Coding Your AI Agents](../sources/youre-wasting-time-coding-your-ai-agents.md) - "[0:05:47] we want to tell our assana assistant what day it is so it can accurately determine due dates when we give relative dates like this Saturday or next Friday"
