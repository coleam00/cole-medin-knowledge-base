...
MCP architecture is a two-sided design: servers expose tools, clients consume them. "Instead of having to Define all of those individual tools and functions we have these services that our mCP servers expose that communicate all these tools to our AI agent." [0:05:36] You can build either half or both: "we can create our own servers to package our own tools and we can also create our own clients so we can build AI agents with Frameworks like pantic AI that can leverage mCP servers." [0:14:31] This clean split is what lets one [MCP](./mcp.md) server serve many different clients.
...
- [Claude MCP has Changed AI Forever - Here's What You NEED to Know](../sources/claude-mcp-has-changed-ai-forever-heres-what-you-need-to-know.md) - "[0:14:31] we can create our own servers to package our own tools and we can also create our own clients so we can build AI agents with Frameworks like pantic AI that can leverage mCP servers"
