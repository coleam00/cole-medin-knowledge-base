...
description: "Build an OKF (Open Knowledge Format) knowledge base from any YouTube channel. Uses Google's OKF standard so any AI can read the output with zero setup. Free, no API key. Uses yt-dlp (most reliable). Invoke with /channel-to-kb-ytdlp @ChannelHandle"
